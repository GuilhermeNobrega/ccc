...
void parse_variable(struct datatype* dtype, struct token* name_token, struct history* history) { // LAB5 - Parte 2
    // TODO: Lidar com a declaracao de vetores. Ex: int a[10].
...
void make_variable_node_and_register(struct history* history, struct datatype* dtype, struct token* name_token, struct node* value_node) {
    ...
    // Empurra para os dois vetores
    node_push(var_node); // vetor de trabalho
    vector_push(node_vector_root, &var_node); // vetor da árvore
    //printf("[DEBUG] VARIABLE node empurrado para node_vector_root: name=%s\n", var_node->var.name);
