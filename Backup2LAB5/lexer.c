...
unsigned long long read_number() {
    ...
    buffer_write(buffer, 0x00);
    const char* val = buffer_ptr(buffer);

    if (is_float) {
        printf("Token número decimal (float): %s\n", val);
        // você pode armazenar esse valor como string no token
        // ou converter com atof() se quiser guardar o valor real
        return atof(val);  // para manter compatível com .llnum (cast)
    } else {
        unsigned long long value = strtoull(val, NULL, 10);
